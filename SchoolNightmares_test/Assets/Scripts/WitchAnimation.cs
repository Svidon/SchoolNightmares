...
public class WitchAnimation : MonoBehaviour
{
    ...
    public void SetDirection(Vector2 _direction) // Vector2.magnitude - returns the length of this vector
    {
    ...
    {
        Vector2 norDir = _direction.normalized; //MARKER return this vector with a magnitude of 1

        float step = 360 / 8; //MARKER 45 one circle and 8 slices // Calculate how many degrees one slice is
        float offset = step / 2; //MAKRERS 22.5 //OFFSET help us easy to calcuate and get the correct index of the string array

        float angle = Vector2.SignedAngle(Vector2.up, norDir); //MARKER returns the signed angle in degress between A and B

        angle += offset; //Help us easy to calcuate and get the correct index of the string array

        if (angle < 0) //avoid negative number
        {
            angle += 360;
        }
